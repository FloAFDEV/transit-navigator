...
// Travel-time color: green (0 min) → amber (half) → red (max)
...
// Simplified JES: perceived cost = travel + transfers*120 + wait*2
// For inter-station Dijkstra paths, transferCount and wait are 0 (not yet tracked).
function computeInterJES(travelSeconds: number) {
  return computeJES(['A', 'B'], travelSeconds, [], 0);
}
...
const StationGraphView: React.FC<Props> = ({ gtfs }) => {
  ...
  // Dijkstra on the inter-station adjacency to find path cost A→B
  const pathResult = useMemo(() => {
    ...
    return { travelSeconds, jes: computeInterJES(travelSeconds) };
  ...
  useEffect(() => {
    ...
    // Deduplicate edges for display (one line per pair)
    ...
    const linkEl = container.append('g').attr('class', 'links')
      ...
      .attr('stroke-width', d => Math.max(0.8, Math.min(3.5, 100 / Math.max(30, d.edge.avgSeconds / 60))))
    ...
    nodeEl.append('text')
      ...
      .attr('pointer-events', 'none')
      .text(d => { const n = d.node.stopName; return n.length > 20 ? n.slice(0, 18) + '…' : n; });
  ...
  const jesInfo = pathResult ? jesLabel(pathResult.jes.normalizedScore) : null;
        ...
        <div className="relative flex-1 rounded-md border border-border bg-card overflow-hidden">
          ...
          {interGraph && interGraph.nodes.size > 0 && (
            <div className="absolute bottom-2 left-2 text-[10px] text-muted-foreground bg-background/80 px-2 py-1 rounded">
              {interGraph.nodes.size} stations · {interGraph.totalStopTimesEdges} connexions
      ...
      <div className="w-72 flex flex-col gap-3 overflow-y-auto">
        ...
        {fromNode && (
          <div className="rounded-md border border-border bg-card p-4">
              ...
              <div className="mt-3 space-y-3">
                <div className="bg-secondary/40 rounded p-3 space-y-1.5 text-[10px]">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{Math.round(pathResult.travelSeconds / 60)} min</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Coût perçu</span>
                    <span className="font-medium">{Math.round(pathResult.jes.rawScore / 60)} min perçues</span>
                  </div>
                </div>
                ...
                <div className="rounded p-3" style={{ backgroundColor: `${jesInfo.color}20`, border: `1px solid ${jesInfo.color}40` }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-bold" style={{ color: jesInfo.color }}>
                      {Math.round(pathResult.jes.normalizedScore)}/100
                    </span>
                  </div>
                  <div className="w-full h-1.5 rounded-full bg-secondary/60 overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${pathResult.jes.normalizedScore}%`, backgroundColor: jesInfo.color }} />
                  </div>
                ...
                {jesImprovementActions(pathResult.jes).length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold mb-1.5">Recommandations</div>
                    <ul className="space-y-1">
                      {jesImprovementActions(pathResult.jes).map((a, i) => (
            ...
            {fromNodeId && toNodeId && !pathResult && (
        ...
        {interGraph && interGraph.nodes.size <= 1 && (
          <div className="rounded-md border border-border bg-card p-4 text-xs text-muted-foreground">
            ...
            <p>Ce stop_id n'apparaît pas dans les horaires. Vérifier le fichier GTFS.</p>
          </div>
        )}
      </div>
    </div>
